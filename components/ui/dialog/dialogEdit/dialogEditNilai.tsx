                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    ...
                    <fieldset className="rounded-lg border border-border p-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            {mapelList.map((mapel) => (
                            <Field key={mapel.id} label={mapel.nama} htmlFor={`edit-nilai-${row.id}-${mapel.id}`} compact>
                                <input
...
function Field({ label, htmlFor, children, className, compact = false }: { label: string; htmlFor: string; children: React.ReactNode; className?: string; compact?: boolean }) {
    return (
        <div className={`flex flex-col gap-1.5 ${className ?? ""}`}>
            <label htmlFor={htmlFor} className={compact ? "truncate text-xs font-medium text-foreground" : "text-sm font-medium text-foreground"}>
                {label}
            </label>
            {children}
        </div>
