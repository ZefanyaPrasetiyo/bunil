...
export default function NilaiSaya() {
    ...
    async function loadData() {
      try {
        ...
        const sessionPayload =
          (await sessionResponse.json()) as UserSessionPayload;
        const user =
          sessionPayload.user ?? sessionPayload.session?.user ?? null;
        ...
        if (active) {
          setStudentName(user.name || user.username || "Siswa");
        }

        const nilaiResponse = await fetch("/api/nilai?page=1&limit=200", {
          credentials: "include",
        });
  ...
  const totalNilai = useMemo(
    () => nilai.reduce((total, item) => total + item.nilai, 0),
    [nilai],
  );
  const nilaiTerkecil = useMemo(
    () => (nilai.length > 0 ? Math.min(...nilai.map((item) => item.nilai)) : 0),
    [nilai],
  );
  const nilaiTerbesar = useMemo(
    () => (nilai.length > 0 ? Math.max(...nilai.map((item) => item.nilai)) : 0),
    [nilai],
  );
  const rataRata = useMemo(
    () => (nilai.length > 0 ? totalNilai / nilai.length : 0),
    [nilai, totalNilai],
  );
    ...
    <div className="flex min-h-svh w-full flex-col gap-8 p-6 font-sans md:p-10">
      <section className="rounded-2xl border border-border bg-card p-6">
        ...
        <p className="mt-2 text-muted-foreground">
          Amati hasil penilaianmu di sini.
        </p>
      </section>
        ...
        <>
          <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard
              ...
              label="Jumlah Keseluruhan Nilai"
              value={totalNilai}
              tone="accent"
            />
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <div className="mt-1 flex items-end gap-3">
                ...
                <span className="mb-1 h-7 w-px bg-border" />
                <div>
          ...
          <section className="flex flex-col gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Daftar nilai untuk setiap mata pelajaran.
              </p>
            </div>
            {nilai.length === 0 ? (
              <div className="rounded-2xl border border-border bg-card p-6 text-sm text-muted-foreground">
              ...
              <div className="flex flex-col gap-3">
                {nilai.map((item) => (
                  ...
                  >
                    ...
                    <span
                      className={`min-w-14 rounded-lg border px-3 py-1.5 text-center font-heading text-lg font-bold ${nilaiTone(item.nilai)}`}
                    >
                      {item.nilai}
                    </span>
                  </article>
...
function StatCard({
  ...
  const toneClass = {
    ...
    secondary:
      "bg-secondary/20 text-[color-mix(in_oklab,var(--secondary),black_35%)] dark:text-secondary",
    accent:
      "bg-accent/20 text-[color-mix(in_oklab,var(--accent),black_45%)] dark:text-accent",
  }[tone];
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div
        className={`mb-4 flex size-11 items-center justify-center rounded-xl ${toneClass}`}
      >
        {icon}
      </div>
