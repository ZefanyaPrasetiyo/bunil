...
const student = { nama: "Ahmad Fauzi" };
const nilai = [
    { pelajaran: "Matematika", nilai: 88 },
    { pelajaran: "Bahasa Indonesia", nilai: 90 },
    { pelajaran: "Bahasa Inggris", nilai: 82 },
    { pelajaran: "IPA", nilai: 85 },
];
...
export default function NilaiSaya() {
    const totalNilai = nilai.reduce((total, item) => total + item.nilai, 0);
    const nilaiTerkecil = Math.min(...nilai.map((item) => item.nilai));
    const nilaiTerbesar = Math.max(...nilai.map((item) => item.nilai));
    ...
    return (
        <div className="flex min-h-svh w-full flex-col gap-8 p-6 font-sans md:p-10">
            <section className="rounded-2xl border border-border bg-card p-6">
                ...
                <h1 className="mt-1 font-heading text-3xl font-bold text-foreground">Hi, {student.nama}</h1>
