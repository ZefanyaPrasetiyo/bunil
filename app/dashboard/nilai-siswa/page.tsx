...
import {
  ...
  type Mapel,
  type NilaiSiswaRow,
} from "@/components/application/table/table-nilaiSiswa";
import {
  DialogTambahNilaiSiswa,
  type NilaiSiswaFormValues,
} from "@/components/ui/dialog/dialogAddNilai";
...
type ListApiResponse<T> = { data?: T[]; message?: string; error?: string };
...
async function getErrorMessage(response: Response) {
  ...
  return body?.error ?? body?.message ?? "Terjadi kesalahan saat memproses nilai siswa.";
}

function buildRows(
  users: UserApi[],
  nilaiRecords: NilaiApi[],
): NilaiSiswaRow[] {
    ...
    .map((user, index) => ({
      ...
      nilai: Object.fromEntries(
        nilaiRecords
          ...
          .map((item) => [item.mapelId, item.nilai]),
      ),
    }));
}

function average(row: NilaiSiswaRow) {
  const values = Object.values(row.nilai).filter(
...
export default function NilaiSiswa() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [usersResponse, mapelResponse, nilaiResponse] = await Promise.all([
        fetch("/api/user?page=1&limit=200", { cache: "no-store" }),
        fetch("/api/mata-pelajaran?page=1&limit=200", { cache: "no-store" }),
        fetch("/api/nilai?page=1&limit=200", { cache: "no-store" }),
      ]);
      if (!usersResponse.ok)
        throw new Error(await getErrorMessage(usersResponse));
      if (!mapelResponse.ok)
        throw new Error(await getErrorMessage(mapelResponse));
      if (!nilaiResponse.ok)
        throw new Error(await getErrorMessage(nilaiResponse));

      const usersPayload = (await usersResponse.json()) as ListApiResponse<UserApi>;
      const mapelPayload = (await mapelResponse.json()) as ListApiResponse<Mapel>;
      const nilaiPayload = (await nilaiResponse.json()) as ListApiResponse<NilaiApi>;

      setUsers(Array.isArray(usersPayload.data) ? usersPayload.data : []);
      setMapelList(Array.isArray(mapelPayload.data) ? mapelPayload.data : []);
      setNilaiRecords(Array.isArray(nilaiPayload.data) ? nilaiPayload.data : []);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Gagal memuat data nilai siswa.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function saveStaging(valuesList: NilaiSiswaFormValues[]) {
    ...
    try {
      await Promise.all(
        valuesList.flatMap((values) => {
          const user = users.find(
            (item) =>
              item.role === "USER" &&
              item.username === values.spmb.trim(),
          );
          if (!user)
            throw new Error(
              `Siswa dengan NO SPMB ${values.spmb} tidak ditemukan.`,
            );
          ...
          return mapelList.flatMap((mapel) => {
            ...
            if (!Number.isFinite(nilai) || nilai < 0 || nilai > 100)
              throw new Error(
                `Nilai ${mapel.nama} harus di antara 0 sampai 100.`,
              );

            const existing = nilaiRecords.find(
              (item) => item.userId === user.id && item.mapelId === mapel.id,
            );
            return [
              fetch(
                existing
                  ? `/api/nilai/${encodeURIComponent(existing.id)}`
                  : "/api/nilai",
                {
                  ...
                  body: JSON.stringify(
                    existing
                      ? { nilai }
                      : { userId: user.id, mapelId: mapel.id, nilai },
                  ),
                },
              ).then(async (response) => {
                if (!response.ok)
  ...
  async function updateNilaiSiswa(
    userId: string,
    values: {
      spmb: string;
      nama: string;
      ...
      await Promise.all(
        mapelList
          .flatMap((mapel) => {
            ...
            const existing = nilaiRecords.find(
              (item) => item.userId === userId && item.mapelId === mapel.id,
            );
            if (value == null)
              return existing
                ? [
                    fetch(`/api/nilai/${encodeURIComponent(existing.id)}`, {
                      method: "DELETE",
                    }),
                  ]
                : [];
            const body = existing
              ? { nilai: value }
              : { userId, mapelId: mapel.id, nilai: value };
            return [
              fetch(
                existing
                  ? `/api/nilai/${encodeURIComponent(existing.id)}`
  ...
  const jurusanOptions = [
    ...new Set(
      data.map((row) => row.jurusan).filter((jurusan) => jurusan !== "-"),
    ),
  ].sort((a, b) => a.localeCompare(b, "id"));
  const filteredData = data
    .filter((row) =>
      [row.nama, row.jurusan, row.spmb]
        .join(" ")
        .toLowerCase()
  ...
  const rataRata =
    data.reduce((sum, row) => sum + average(row), 0) / (totalSiswa || 1);
    ...
    <div className="flex min-h-svh w-full flex-col gap-6 p-6 font-sans md:p-10">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <StatCard
          ...
          value={mapelList.length}
        />
        <StatCard
          icon={<GraduationCap className="size-5" />}
          tone="accent"
          label="Rata-rata Nilai"
          value={rataRata.toFixed(1)}
        />
      </div>
        ...
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
          <label className="inline-flex items-center gap-2 rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground">
            ...
            <select
              ...
              value={jurusanFilter}
              onChange={(event) => setJurusanFilter(event.target.value)}
      ...
      {isLoading ? (
        ...
        <TableNilaiSiswa
          mapelList={mapelList}
          data={filteredData}
          onEdit={updateNilaiSiswa}
          onDelete={deleteNilaiSiswa}
        />
      )}
    </div>
  );
